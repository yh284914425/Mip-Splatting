...
def readColmapCameras(cam_extrinsics, cam_intrinsics, images_folder):
    ...
    for idx, key in enumerate(cam_extrinsics):
        ...
        sys.stdout.write("正在读取相机 {}/{}".format(idx+1, len(cam_extrinsics)))
        sys.stdout.flush()
        ...
        if intr.model=="SIMPLE_PINHOLE":
            ...
            assert False, "不支持Colmap相机模型：仅支持无畸变数据集（PINHOLE或SIMPLE_PINHOLE相机）！"
...
def readColmapSceneInfo(path, images, eval, llffhold=8):
    ...
    if not os.path.exists(ply_path):
        print("正在将point3d.bin转换为.ply，这只会在您第一次打开场景时发生。")
        try:
...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            # NeRF的“transform_matrix”是相机到世界的变换
            c2w = np.array(frame["transform_matrix"])
            # 从OpenGL/Blender相机轴（Y向上，Z向后）更改为COLMAP（Y向下，Z向前）
            c2w[:3, 1:3] *= -1

            # 获取世界到相机的变换并设置R，T
            w2c = np.linalg.inv(c2w)
            R = np.transpose(w2c[:3,:3])  # 由于CUDA代码中的“glm”，R被存储为转置
            T = w2c[:3, 3]
...
def readNerfSyntheticInfo(path, white_background, eval, extension=".png"):
    ...
    if not os.path.exists(ply_path):
        ...
        num_pts = 100_000
        print(f"正在生成随机点云 ({num_pts})...")
        
        # 我们在合成Blender场景的边界内创建随机点
...
def readMultiScale(path, white_background,split, only_highres=False):
    ...
    # 现在应该在self.meta中包含['pix2cam'，'cam2world'，'width'，'height']
...
sceneLoadTypeCallbacks = {
    ...
    "多尺度": readMultiScaleNerfSyntheticInfo,
}
